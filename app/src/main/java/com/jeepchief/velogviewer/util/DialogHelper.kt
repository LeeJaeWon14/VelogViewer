...
import com.jeepchief.velogviewer.databinding.LayoutProgressDialogBinding
...
object DialogHelper {
     ...
     * @param message if it is null, disable message(TextView)
     *
     */
    @Synchronized
    fun progressDialog(
        ...
        return progressDlg ?: run {
            progressDlg = AlertDialog.Builder(context).create().apply {
                ...
                setCancelable(false)
                message?.let {
                    view.tvProgressMsg.text = message
                } ?: run {
